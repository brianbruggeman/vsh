...
def dumps(o, encoder: Optional["TomlEncoder"] = None) -> str:
...
def _dump_float(v: float) -> str:
    return "{0:.16}".format(v).replace("e+0", "e+").replace("e-0", "e-")


def _dump_time(v: datetime.datetime) -> str:
...
class TomlEncoder:
    ...
    def dump_list(self, v: List) -> str:
        retval = f'[{", ".join(self.dump_value(u) for u in v)}]'
        return retval

    def dump_inline_table(self, section: Any) -> str:
